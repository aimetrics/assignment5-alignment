...
def compute_group_normalized_rewards(
    ...
    Args:
        reward_fn: Callable[[response, ground_truth], dict] returning keys: "reward", "format_reward", "answer_reward"
        rollout_responses: list[str], length = rollout_batch_size. for each response, generate a candidate answer.
...
def compute_naive_policy_gradient_loss(
    ...
    """
    把每条样本的一个标量奖励/优势，乘到这条样本所有 token 的 log-prob 上，形成最基础的 policy gradient loss.
    “naive”的原因：同一条回答中的所有 token，共享同一个序列级别的 advantage，没有做 token-level credit assignment.
    Compute per-token naive policy gradient loss:
        loss_{b,t} = -A_b * log_prob_{b,t}
        b 是第几个样本, t 是第几个 token, A_b 是这个样本的奖励/优势, log_prob_{b,t} 是当前策略对这个样本的第 t 个 token 的 log probability.

        这是最经典的REINFORCE算法，直接把每个 token 的 log-prob 乘以一个标量奖励/优势，形成一个标量损失。

    Args:
        raw_rewards_or_advantages: Tensor of shape (batch_size, 1). 最原始的回报, sequence level reward
        policy_log_probs: Tensor of shape (batch_size, sequence_length). 当前策略对每个 token 的 log probability

    Returns:
        Tensor of shape (batch_size, sequence_length)
        
        这个函数只负责算 per-token loss，不负责最后聚合。为什么返回的是 (B, T) 而不是一个标量?
        返回 (B, T) 有几个好处：
        - 后面可以结合 response_mask，只对 response token 求平均
        - 可以按 token、按样本、按 batch 灵活归约
        - 和后面的 masked_mean、microbatch train step 更好衔接
        - 它是一个“底层积木函数”，而不是最终训练 loss。
...
def compute_grpo_clip_loss(
    ...
    """
    同一句话里的 token，更新幅度不一致, 由 ratio(exp(logπ_new - logπ_old)) 决定每个 token 的相对变化幅度. 每个 token 的 log_prob 都不一样. clip 也是 token level
    PPO/GRPO 实际在做：对每个 token 做“trust region control”
...
def compute_policy_gradient_loss(
    ...
    Args:
        policy_log_probs: (B, T) token-level log-probabilities from current policy.
...
def masked_mean(
    ...
    """
    在只考虑有效 token 的前提下，对 token-level loss 做平均，使每个样本在训练中拥有相同权重，避免长度带来的偏差。
...
def masked_normalize(
    ...
    """
    masked_normalize = masked sum / 固定常数，它解决的是“统一尺度”问题，不是“按有效长度求平均”问题. 统一尺度来自 constant_normalizer.
    和maked_mean唯一区别:
    - masked_mean      = sum / 有效token数          -> 每个样本权重一样, 每个样本等权.
    - masked_normalize = sum / constant_normalizer -> token 数量越多 → loss 越大, 每个token等权. 长回答会 dominate 梯度, 模型更关注长回答.
